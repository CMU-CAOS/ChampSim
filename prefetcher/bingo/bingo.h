...
#include <sstream>
#include <algorithm>
#include <iostream> // ported: Pythia got cerr transitively via its headers

#include "champsim.h"
#include "modules.h"
#include "cache.h"    // ported: complete CACHE type for intern_->get_pq_*/get_mshr_*
#include "bakshalipour_framework.h"

using namespace std;

// --- Ported from Pythia to the champsim::modules API ---
// Pythia's cache.h supplied these fill-level constants. Keep them >0 (0 in a
// pattern means "do not prefetch this block"). At issue time an L2-placed Bingo
// maps them to the modern bool prefetch_line(fill_this_level): L1/L2 targets
// fill this (L2) level, LLC targets fill lower only.
#ifndef FILL_L1
#define FILL_L1 1
#define FILL_L2 2
#define FILL_LLC 3
...
// Pythia configured Bingo via knob:: (config/bingo.ini). Those are now
// compile-time constants, so the .cc keeps referencing knob::bingo_* unchanged.
namespace knob
{
  constexpr uint32_t bingo_region_size = 2048;
  constexpr uint32_t bingo_pattern_len = 32;
  constexpr uint32_t bingo_pc_width = 16;
  constexpr uint32_t bingo_min_addr_width = 5;
  constexpr uint32_t bingo_max_addr_width = 16;
  constexpr uint32_t bingo_ft_size = 64;
  constexpr uint32_t bingo_at_size = 128;
  constexpr uint32_t bingo_pht_size = 4096;
  constexpr uint32_t bingo_pht_ways = 16;
  constexpr uint32_t bingo_pf_streamer_size = 128;
  constexpr uint32_t bingo_debug_level = 0;
  constexpr float bingo_l1d_thresh = 1.01f;
  constexpr float bingo_l2c_thresh = 0.80f;
  constexpr float bingo_llc_thresh = 0.05f;
}
...
public:
   ...
   // Pythia passed the CACHE* and issued via cache->prefetch_line, gating on
   // cache->PQ/MSHR occupancy. The port passes the owning module and reaches the
   // same queues through owner->intern_, preserving Pythia's exact pre-issue check.
   int prefetch(champsim::modules::prefetcher *owner, uint64_t block_address) {
      ...
      for (int d = 1; d < this->pattern_len; d += 1) {
         ...
         for (int sgn = +1; sgn >= -1; sgn -= 2) {
            ...
            if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
               ...
               // Pythia's exact pre-issue gate, via the modern queue accessors.
               auto *c = owner->intern_;
               std::size_t pq_occ = c->get_pq_occupancy().front();
               std::size_t pq_size = c->get_pq_size().front();
               std::size_t mshr_occ = c->get_mshr_occupancy();
               std::size_t mshr_size = c->get_mshr_size();
               if (pq_occ + mshr_occ < mshr_size - 1 && pq_occ < pq_size) {
                  // FILL_LLC -> fill lower level only; L1/L2 -> fill this (L2) level.
                  owner->prefetch_line(champsim::address{pf_address}, pattern[pf_offset] != FILL_LLC, 0);
                  pf_issued += 1;
                  pattern[pf_offset] = 0;
               } else {
...
public:
   ...
   ~bingo();
   // champsim::modules::prefetcher hooks (replace Pythia's ctor(type,cache) /
   // invoke_prefetcher / register_fill / dump_stats).
