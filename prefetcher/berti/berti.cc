...
void berti::prefetcher_initialize()
{
  ...
  // New structures
  picturePF foo;
  foo.latencyt = new LatencyTable(static_cast<int>(latency_table_size));
  foo.scache = new ShadowCache(static_cast<int>(intern_->NUM_SET), static_cast<int>(intern_->NUM_WAY));
...
uint32_t berti::prefetcher_cache_operate(champsim::address addr_a, champsim::address ip_a,
                                         uint8_t cache_hit, bool useful_prefetch,
                                         access_type type, uint32_t metadata_in)
{
  // champsim::address -> raw byte addresses; body below is unchanged.
  uint64_t addr = addr_a.to<uint64_t>();
  uint64_t ip = ip_a.to<uint64_t>();

  // We select the structures for every cpu
  latencyt = bigPicture[intern_->cpu].latencyt;
  scache = bigPicture[intern_->cpu].scache;
  historyt = bigPicture[intern_->cpu].historyt;
  berti_ptr = bigPicture[intern_->cpu].berti;
  ...
  uint64_t ip_hash = berti_ptr->ip_hash(ip) & IP_MASK;
  ...
  if (!cache_hit) // This is a miss
  {
    ...
    latencyt->add(line_addr, ip_hash, false, intern_->current_cycle()); // Add @ to latency
    historyt->add(ip_hash, line_addr, intern_->current_cycle()); // Add to the table
  } else if (cache_hit && scache->is_pf(line_addr)) // Hit bc prefetch
  {
    ...
    berti_ptr->find_and_update(latency, ip_hash, intern_->current_cycle() & TIME_MASK, line_addr);
    historyt->add(ip_hash, line_addr, intern_->current_cycle() & TIME_MASK);
  } else
  {
    if constexpr (champsim::debug_print) 
...
# endif
    ...
    if (prefetch_line(champsim::address{p_addr}, fill_this_level, metadata_in))
    {
      ...
      if (fill_this_level)
      {
        ...
        {
          latencyt->add(p_b_addr, ip_hash, true, intern_->current_cycle());
        }
      }
    }
  }
...
uint32_t berti::prefetcher_cache_fill(champsim::address addr_a, long set, long way,
                                      ...
                                      uint32_t metadata_in)
{
  ...
  {
    berti_ptr->find_and_update(latency, tag, cycle, line_addr);
  }
  return metadata_in;
}
