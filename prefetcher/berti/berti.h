...
namespace berti_space
{
  ...
  inline welford_t average_latency;
  
  // Get more info
  inline uint64_t pf_to_l1 = 0;
  inline uint64_t pf_to_l2 = 0;
  inline uint64_t pf_to_l2_bc_mshr = 0;
  inline uint64_t cant_track_latency = 0;
  inline uint64_t cross_page = 0;
  inline uint64_t no_cross_page = 0;
  inline uint64_t no_found_berti = 0;
  ...
  // Global "current" table pointers (reassigned per access from bigPicture).
  // Renamed berti -> berti_ptr so it does not collide with the module class
  // name 'berti' inside its member functions.
  inline LatencyTable *latencyt;
  inline ShadowCache *scache;
  inline HistoryTable *historyt;
  inline Berti *berti_ptr;

  // This is structure is an adaption of Berti for multicore simulations
  typedef struct picturePF {
  ...
  inline std::vector<picturePF_t> bigPicture;
};

// Module wrapper for the class-based champsim::modules::prefetcher API.
// The prefetcher state lives in berti_space (file-scope) as in the original;
// this class only provides the entry points ChampSim calls.
