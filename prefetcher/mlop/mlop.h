...
// Ported: Pythia's cache.h supplied these fill levels (keep >0). mlop sits at L2;
// at issue they map to the modern bool prefetch_line(fill_this_level): L1/L2 ->
// this (L2) level, LLC -> fill lower only.
#ifndef FILL_L1
#define FILL_L1 1
#define FILL_L2 2
#define FILL_LLC 3
...
// config/mlop.ini values as constants (replacing Pythia's knob:: system).
namespace knob
{
  constexpr uint32_t mlop_pref_degree = 16;
  constexpr uint32_t mlop_num_updates = 500;
  constexpr float mlop_l1d_thresh = 2.0f;
  constexpr float mlop_l2c_thresh = 0.75f;
  constexpr float mlop_llc_thresh = 0.3f;
  constexpr uint32_t mlop_debug_level = 0;
}
...
class mlop : public champsim::modules::prefetcher
{
private:
...
public:
	...
	void prefetch(champsim::modules::prefetcher *owner, uint64_t block_number);
