...
// Pythia declared the mlop_* knobs as externs; the port defines them as
// constants in mlop.h, so this extern block is removed.
...
void mlop::init_knobs()
{
	...
	blocks_in_cache = intern_->NUM_SET * intern_->NUM_WAY;
...
mlop::mlop(CACHE *cache) : champsim::modules::prefetcher(cache)
{
	init_knobs();
	init_stats();
 ...
 */
// Pythia passed the CACHE*; the port passes the owning module and reaches the
// cache through owner->intern_.
void mlop::prefetch(champsim::modules::prefetcher *owner, uint64_t block_number) {
	if (this->debug_level >= 2) {
		cout << "mlop::prefetch(block_number=0x" << hex << block_number << dec << ")" << endl;
	}
	int pf_issued = 0;
	uint64_t zone_number = block_number / this->blocks_in_zone;
	int zone_offset = block_number % this->blocks_in_zone;
	...
	for (uint32_t d = 0; d < PF_DEGREE; d += 1) {
		for (auto &cur_pf_offset : this->pf_offset[d]) {
			...
			auto *c = owner->intern_; // Pythia's PQ+MSHR pre-issue gate, via modern accessors
			if (this->is_inside_zone(offset_to_prefetch) && c->get_pq_occupancy().front() < c->get_pq_size().front() &&
					c->get_pq_occupancy().front() + c->get_mshr_occupancy() < c->get_mshr_size() - 1) {
				...
				owner->prefetch_line(champsim::address{pf_addr}, this->pf_level[d] != FILL_LLC, 0);
				// assert(ok == 1);
...
void mlop::prefetcher_final_stats()
{
	print_stats();
}

/* Base-class virtual function */
uint32_t mlop::prefetcher_cache_operate(champsim::address addr_, champsim::address ip_, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                        uint32_t metadata_in)
{
    ...
    /* check prefetch hit: Pythia read parent->block[set][way].prefetch; the modern
       hook provides it directly as useful_prefetch. */
    bool prefetch_hit = useful_prefetch;
...
uint32_t mlop::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr_, uint32_t metadata_in)
{
	uint64_t evicted_addr = evicted_addr_.to<uint64_t>();
	// Pythia read parent->block[set][way].valid; modern hook gives evicted_addr (0 = none).
	if (evicted_addr == 0)
