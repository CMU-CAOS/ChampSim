...
// Pythia ran this from the constructor(type, cache); champsim::modules calls
// prefetcher_initialize() once at startup.
void ipcp_L1::prefetcher_initialize()
{
   init_knobs();
   init_stats();
   print_config();
...
uint32_t ipcp_L1::prefetcher_cache_operate(champsim::address addr_, champsim::address ip_, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                           uint32_t metadata_in)
{
   // Pythia's invoke_prefetcher took raw uint64 ip/addr; recover them here.
   ...
   if(num_misses[cpu] == 256){
       mpkc[cpu] = ((float) num_misses[cpu]/(intern_->current_cycle()-prev_cpu_cycle[cpu]))*1000;
       prev_cpu_cycle[cpu] = intern_->current_cycle();
...
uint32_t ipcp_L1::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
   return metadata_in;
}

void ipcp_L1::prefetcher_final_stats()
{
