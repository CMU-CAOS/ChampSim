...
{
protected:
	champsim::modules::prefetcher *m_parent;
	float m_alpha;
	float m_gamma;
	float m_epsilon;
	uint32_t m_actions;
...
public:
	LearningEngineBase(champsim::modules::prefetcher *p, float alpha, float gamma, float epsilon, uint32_t actions, uint32_t states, uint64_t seed, std::string policy, std::string type);
