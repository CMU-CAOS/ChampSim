...
// Ported: these came from Pythia's cache.h / champsim.h (histogram dimensions).
#ifndef CACHE_ACC_LEVELS
#define CACHE_ACC_LEVELS 10
#endif
#ifndef DRAM_BW_LEVELS
...
class scooby : public champsim::modules::prefetcher
{
private:
...
public:
	explicit scooby(CACHE *cache);
	~scooby();
	void prefetcher_initialize();
	// champsim::modules::prefetcher hooks (replace Pythia's ctor(type) /
	// invoke_prefetcher / dump_stats). Scooby returns prefetches in a vector that
	// Pythia's wrapper issued; here we issue them via prefetch_line at L2.
