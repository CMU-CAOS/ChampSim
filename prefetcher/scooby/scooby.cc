...
#include <algorithm>
#include <iomanip>
#include <iostream> // ported: Pythia got cout transitively via its headers
#include "champsim.h"
#include "cache.h"
#include "scooby.h"
#include "util.h"
...
scooby::scooby(CACHE *cache) : champsim::modules::prefetcher(cache)
{
	// Only null the owning pointers here. The champsim::modules system stores the
	// prefetcher by value (std::tuple<scooby>) and copies/moves it during cache
	// setup; allocating the raw-pointer-owned learning engines in the constructor
	// let a temporary's destructor free them, leaving the stored instance dangling
	// (heap-use-after-free). Allocation moved to prefetcher_initialize(), which
	// ChampSim calls once on the final stored instance.
	recorder = NULL;
...
void scooby::prefetcher_initialize()
{
	init_knobs();
	init_stats();
...
uint32_t scooby::prefetcher_cache_operate(champsim::address addr_, champsim::address ip_, uint8_t cache_hit, bool useful_prefetch, access_type type_,
                                          uint32_t metadata_in)
{
	// Pythia's invoke_prefetcher took raw uint64 pc/address and an output vector the
	// cache issued; recover the integers and collect prefetches locally.
	uint64_t address = addr_.to<uint64_t>();
	uint64_t pc = ip_.to<uint64_t>();
	uint8_t type = 0;            // Pythia access-type enum, only used in disabled MYLOG
	vector<uint64_t> pref_addr;
	if (useful_prefetch)         // Pythia's cache called this on a demand hit to a prefetched line
		register_prefetch_hit(address);
	...
	/* Pythia's multi.l2c_pref issued scooby's returned pref_addr with FILL_L2.
	   Issue here: fill this (L2) level, prefetch metadata 0. */
...
// champsim::modules fill hook -> Pythia's register_fill(address).
uint32_t scooby::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void scooby::prefetcher_final_stats()
{
