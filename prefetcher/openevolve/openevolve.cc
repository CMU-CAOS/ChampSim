#include "openevolve.h"

#include <iostream>

void openevolve::prefetcher_initialize()
{
...
void openevolve::prefetcher_cycle_operate() {}

uint32_t openevolve::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                           uint32_t metadata_in)
{
  ...
  typename openevolve::offset_type::difference_type delta = 0;
  std::vector<typename openevolve::offset_type::difference_type> delta_q(intern_->MSHR_SIZE);
  ...
  confidence_q[0] = 100;
  global_history_register_reads++;
  global_history_register_writes++;
  ...
  FILTER.check(addr, openevolve::L2C_DEMAND);
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if (champsim::page_number{pf_addr} == page) { // Prefetch request is in the same physical page
          if (FILTER.check(pf_addr, ((confidence_q[i] >= FILL_THRESHOLD) ? openevolve::SPP_L2C_PREFETCH : openevolve::SPP_LLC_PREFETCH))) {
          ...
          if constexpr (GHR_ON) {
            ...
            GHR.update_entry(curr_sig, confidence_q[i], openevolve::offset_type{pf_addr}, delta_q[i]);
          }
        }
...
uint32_t openevolve::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    if constexpr (SPP_DEBUG_PRINT) {
      std::cout << std::endl;
    }
    FILTER.check(evicted_addr, openevolve::L2C_EVICT);
  }

  return metadata_in;
}

void openevolve::prefetcher_final_stats()
{
  std::cout << "signature_table: r=" << signature_table_reads << " w=" << signature_table_writes << std::endl;
  std::cout << "pattern_table: r=" << pattern_table_reads << " w=" << pattern_table_writes << std::endl;
  std::cout << "prefetch_filter: r=" << prefetch_filter_reads << " w=" << prefetch_filter_writes << std::endl;
  std::cout << "global_history_register: r=" << global_history_register_reads << " w=" << global_history_register_writes << std::endl;
}

// TODO: Find a good 64-bit hash function
uint64_t openevolve::get_hash(uint64_t key)
{
...
void openevolve::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig, typename offset_type::difference_type& delta)
{
  _parent->signature_table_reads++;
  ...
  // Case 3: All entries valid, no tag match — LRU replacement
  if (match == ST_WAY) {
    for (match = 0; match < ST_WAY; match++) {
    ...
    // Fallback: if LRU counters are corrupted, evict way 0
    if (match == ST_WAY) {
      match = 0;
      tag[set][match] = partial_page;
      sig[set][match] = 0;
  ...
  for (uint32_t way = 0; way < ST_WAY; way++) {
    if (lru[set][way] < lru[set][match]) {
      ...
      if (lru[set][way] >= ST_WAY)
        lru[set][way] = ST_WAY - 1;
    }
  }

  lru[set][match] = 0; // Promote to the MRU position
  _parent->signature_table_writes++;
}

void openevolve::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
  _parent->pattern_table_reads++;
  _parent->pattern_table_writes++;
  ...
  if (match == PT_WAY) {
    ...
    // Fallback: if no victim found, evict way 0
    if (victim_way == PT_WAY)
      victim_way = 0;
  }
}

void openevolve::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q, std::vector<uint32_t>& confidence_q,
...
bool openevolve::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  _parent->prefetch_filter_reads++;
  ...
  case openevolve::SPP_L2C_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      _parent->prefetch_filter_writes++;
  ...
  case openevolve::SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      if constexpr (SPP_DEBUG_PRINT) {
        std::cout << "[FILTER] " << __func__ << " line is already in the filter check_addr: " << check_addr << " cache_line: " << cache_line;
        std::cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << std::endl;
  ...
  case openevolve::L2C_DEMAND:
    if ((remainder_tag[quotient] == remainder) && (useful[quotient] == 0)) {
      useful[quotient] = 1;
      _parent->prefetch_filter_writes++;
      if (valid[quotient]) {
        _parent->global_history_register_writes++;
...
void openevolve::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta)
{
  _parent->global_history_register_reads++;
  ...
  // Fallback: if no victim found, evict entry 0
...
uint32_t openevolve::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
  _parent->global_history_register_reads++;
