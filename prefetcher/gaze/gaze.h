...
// BLOCK_SIZE/LOG2_BLOCK_SIZE are runtime `extern const` in this ChampSim (not
// constexpr), so these are `const` and stay derived from the configured block size.
...
const int PT_SIZE = PT_WAY * NUM_BLOCKS; // const: depends on NUM_BLOCKS (runtime const)
...
public:
    ...
    void prefetch(champsim::modules::prefetcher* cache, uint64_t block_num);
...
public:
    ...
    void access(uint64_t block_num, uint64_t ip, champsim::modules::prefetcher* cache);
    ...
    void tune_stride_degree(champsim::modules::prefetcher* cache);
...
// champsim::modules::prefetcher wrapper. Replaces Gaze's DPC3 CACHE:: hooks and
// per-cpu `prefetchers` vector with one Gaze instance owned by the module.
class gaze : public champsim::modules::prefetcher {
    gzp::Gaze impl;

public:
    explicit gaze(CACHE* cache);
