...
// From Gaze's cache.h: pack the prefetch source/destination cache level into the
// prefetch metadata bits (Gaze's own convention).
#ifndef __add_pf_dest_level
...
void PrefetchBuffer::insert(uint64_t region_num, std::vector<int> pattern, uint64_t trigger, uint64_t second, uint32_t pf_metadata) {
    ...
    if ((pf_metadata & 3) == 0 || (pf_metadata & 3) == 3) { // stride & promote
        auto entry = find(key);
        if (!entry) {
            Super::insert(key, {pattern, trigger, trigger, std::vector<int>(gzp::NUM_BLOCKS, pf_metadata)});
            Super::rp_insert(key);
        } else {
        ...
        Super::insert(key, {pattern, trigger, second, std::vector<int>(gzp::NUM_BLOCKS, pf_metadata)});
        Super::rp_insert(key);
    }
}

void PrefetchBuffer::prefetch(champsim::modules::prefetcher* cache, uint64_t block_num) {
    ...
    for (uint64_t i = 1; i < (REGION_SIZE / BLOCK_SIZE); i++) {
        ...
        if (pf_offset != trigger && pf_offset != second && pattern[pf_offset] != 0) {
            ...
            // DPC3 get_occupancy(3=PQ,0=MSHR)/get_size -> modern accessors (deprecated
            // get_occupancy returns 0 for PQ in this tree, which would break the gate).
            auto* c = cache->intern_;
            if (c->get_pq_occupancy().front() + c->get_mshr_occupancy() < c->get_mshr_size() - 1 && c->get_pq_occupancy().front() < c->get_pq_size().front()) {
                ...
                int ok = cache->prefetch_line(champsim::address{pf_addr}, pattern[pf_offset] == PF_FILL_L1 ? true : false, pf_metadata);
...
void Gaze::access(uint64_t block_num, uint64_t pc, champsim::modules::prefetcher* cache) {
    uint64_t region_num = block_num >> (LOG2_REGION_SIZE - LOG2_BLOCK_SIZE);
    uint64_t region_offset = __region_offset(block_num);
...
void Gaze::prefetch(champsim::modules::prefetcher* cache, uint64_t block_num) {
...
void Gaze::tune_stride_degree(champsim::modules::prefetcher* cache) {}
...
// ---- champsim::modules::prefetcher wrapper (replaces the DPC3 CACHE:: hooks) ----

// Gaze has value-member tables (no owning raw pointers), so it copies/moves
// safely with the module — construct impl here (parameterized ctor; Gaze has no
// usable default ctor). Was: prefetchers = vector<Gaze>(NUM_CPUS, Gaze(...)).
gaze::gaze(CACHE* cache)
    : champsim::modules::prefetcher(cache)
    , impl(gzp::FT_SIZE, gzp::FT_WAY, gzp::AT_SIZE, gzp::AT_WAY, gzp::PT_SIZE, gzp::PT_WAY, gzp::PB_SIZE, gzp::PB_WAY, 0)
{
    std::cout << intern_->NAME << " Gaze prefetcher" << std::endl;
}

uint32_t gaze::prefetcher_cache_operate(champsim::address addr_, champsim::address ip_, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                        uint32_t metadata_in)
{
    uint64_t addr = addr_.to<uint64_t>();
    uint64_t ip = ip_.to<uint64_t>();

    impl.set_warmup(intern_->warmup);

    if (type != access_type::LOAD)
        return metadata_in;
    uint64_t block_num = addr >> LOG2_BLOCK_SIZE;

    // `this` (a champsim::modules::prefetcher*) is the "cache" Gaze threads through
    // to issue via prefetch_line and read queue occupancy via intern_.
    impl.access(block_num, ip, this);
...
void gaze::prefetcher_final_stats() { impl.log(); }