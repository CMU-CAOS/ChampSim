...
void to_json(nlohmann::json& j, const CACHE::stats_type& stats)
{
  ...
  statsmap.emplace("tag read", stats.tag_read.total());
  statsmap.emplace("tag write", stats.tag_write.total());
  statsmap.emplace("data read", stats.data_read.total());
  statsmap.emplace("data write", stats.data_write.total());
