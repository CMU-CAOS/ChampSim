  ...
  while (std::any_of(std::begin(scheduled_order), std::end(scheduled_order), [](auto x) { return x == 0; })) {
    ...
    while (chunk_begin != chunk_end) {
      ...
      if (chunk_begin != chunk_end) {
        scheduled_order[static_cast<uint64_t>(std::distance(std::begin(next_scheduled), chunk_begin))] =
            static_cast<uint64_t>(uut->current_time.time_since_epoch() / (uut->clock_period));
        break;
      }
    }
...
SCENARIO("A series of reads arrive at the memory controller and are reordered")
{
  GIVEN("A request stream to the memory controller")
  {
    ...
    MEMORY_CONTROLLER uut{champsim::modules::ModuleBuilder{"uut", "DRAM", champsim::defaults::default_memory_controller()}
                              ...
                              .add_parameter("n_rcd", trcd_cycles)
                              .add_parameter("n_cas", tcas_cycles)
                              .add_parameter("n_ras", tras_cycles)
    ...
    std::vector<uint64_t> cycles_for_first_bank_access = {arriv_time[2],  1 /*arriv_time[3]*/, arriv_time[6], arriv_time[9],
                                                          arriv_time[12], arriv_time[15],      arriv_time[19]};
    ...
    std::vector<uint64_t> cycles_for_third_bank_access = {start_after_second_bank_access + 1 * (trp_cycles + trcd_cycles) + bankgroup_reaccess_delay_l * 4,
    ...
    std::vector<uint64_t> expected_cycles = {cycles_for_second_bank_access[0], cycles_for_third_bank_access[0],  cycles_for_first_bank_access[0],
    ...
    for (uint64_t i = 0; i < row_access.size(); i++) {
      ...
      uint64_t offset = 0;
      champsim::address_slice block_slice{champsim::dynamic_extent{champsim::data::bits{champsim::lg2(chan_size * pref_size)}, champsim::data::bits{offset}},
                                          0};
      offset += champsim::lg2(chan_size * pref_size);
      champsim::address_slice channel_slice{champsim::dynamic_extent{champsim::data::bits{champsim::lg2(DRAM_CHANNELS) + offset}, champsim::data::bits{offset}},
                                            0};
      offset += champsim::lg2(DRAM_CHANNELS);
      champsim::address_slice bankgroup_slice{
          champsim::dynamic_extent{champsim::data::bits{champsim::lg2(DRAM_BANKGROUPS) + offset}, champsim::data::bits{offset}}, bakg_access[i]};
      offset += champsim::lg2(DRAM_BANKGROUPS);
      champsim::address_slice bank_slice{champsim::dynamic_extent{champsim::data::bits{champsim::lg2(DRAM_BANKS) + offset}, champsim::data::bits{offset}},
                                         bak_access[i]};
      offset += champsim::lg2(DRAM_BANKS);
      champsim::address_slice column_slice{
          champsim::dynamic_extent{champsim::data::bits{champsim::lg2(DRAM_COLUMNS / PREFETCH_SIZE) + offset}, champsim::data::bits{offset}}, col_access[i]};
